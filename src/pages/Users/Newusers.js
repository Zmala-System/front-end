...
function Newusers() {
  ...
  const [deletePrisoner, { loading: deleteLoading, error: deleteError }] = useMutation(
  ...
  if (loading) {
    return <div>
      ...
      Loading prisoners...</div>; // Display loading message
  }
  ...
  if (error) {
    return <div>
      ...
      Error fetching prisoners: {error.message}</div>; 
  }
