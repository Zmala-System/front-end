  ...
  useEffect(() => {
    if (incomingData) {
      const [prisonerId, prisonerName, lat, lon, battery, alert1, alert2] =
        incomingData.split("/");
      const newAlerts = [];
      if (alert1 !== "None") newAlerts.push(alert1);
      if (alert2 !== "None") newAlerts.push(alert2);

      setAlerts((prevAlerts) => {
        const combinedAlerts = [...prevAlerts, ...newAlerts];
        return combinedAlerts.slice(-2);
      });
    }
  }, [incomingData]);
          ...
          >
            {alerts.length > 0 ? (
              alerts.map((alert, index) => (
                <div
                  key={index}
                  className="text-gray-800 bg-red-500 rounded-lg p-2 w-full text-center p-2"
                >
                  {alert}
                </div>
              ))
